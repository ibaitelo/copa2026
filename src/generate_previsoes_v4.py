"""
generate_previsoes_v4.py — Gera copa2026_previsoes_v4.xlsx com dados do modelo v7 (corrigido opp_saves_decay).

Recria a estrutura do arquivo original (05/jun/2026) com:
  - "Fase de Grupos"      : 48 jogos com λ, V%/E%/D%, top-3 placares (v7)
...
OUT_XLSX = Path("outputs/copa2026_previsoes_v4.xlsx")
...
def build_grupos_rows(lam_cache: dict, elo: dict) -> list[list]:
    rows: list[list] = []
    rows.append(["COPA DO MUNDO 2026 — PREVISÃO DE PLACARES  (modelo XGBoost v7 — pós-correção opp_saves_decay)"] + [None] * 14)
...
def build_matamata_rows(lam_cache: dict, elo: dict,
    ...
    rows: list[list] = []
    rows.append(["COPA DO MUNDO 2026 — MATA-MATA DETERMINÍSTICO  (modelo XGBoost v7 — pós-correção opp_saves_decay)"] + [None] * 14)
