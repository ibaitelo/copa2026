"""
generate_previsoes_v6.py — Gera copa2026_previsoes_v6.xlsx com dados do modelo v9.

Estrutura:
  - "Fase de Grupos"        : 48 jogos com λ, V%/E%/D%, top-3 placares (v9)
  - "Mata-Mata"             : bracket determinístico v9
  - "Classificação Esperada": esperança analítica de pts/GF/GA por grupo
  - "Probabilidades MC"     : probabilidades Monte Carlo (10k simulações)
"""
...
OUT_XLSX = Path("outputs/copa2026_previsoes_v6.xlsx")
...
def build_grupos_rows(lam_cache: dict, elo: dict) -> list[list]:
    rows: list[list] = []
    rows.append(["COPA DO MUNDO 2026 — PREVISÃO DE PLACARES  (modelo XGBoost v9 — ELO-based gls_ponderado + forte threshold)"] + [None] * 14)
...
def build_matamata_rows(lam_cache: dict, elo: dict,
    ...
    rows: list[list] = []
    rows.append(["COPA DO MUNDO 2026 — MATA-MATA DETERMINÍSTICO  (modelo XGBoost v9 — ELO-based gls_ponderado + forte threshold)"] + [None] * 14)
...
def build_probabilidades_rows(mc_csv: Path) -> list[list]:
    """Aba extra com probabilidades Monte Carlo por fase (10k simulações)."""
    if not mc_csv.exists():
        return []

    mc = pd.read_csv(mc_csv)
    rows: list[list] = []
    rows.append(["COPA DO MUNDO 2026 — PROBABILIDADES MONTE CARLO  (10.000 simulações — modelo v9)"] + [None] * 8)
...
def write_probabilidades(ws, rows: list[list]) -> None:
    if not rows:
        return
    n_cols = 10
    ...
    FILLS_RANK = [
        PatternFill("solid", fgColor="1A7340"),  # 1
        PatternFill("solid", fgColor="217A43"),
        PatternFill("solid", fgColor="2E8A50"),
        PatternFill("solid", fgColor="3D9960"),
        PatternFill("solid", fgColor="52A870"),
        PatternFill("solid", fgColor="6ABF85"),
        PatternFill("solid", fgColor="85D09A"),
        PatternFill("solid", fgColor="A3E0B0"),
        PatternFill("solid", fgColor="C2EEC7"),
        PatternFill("solid", fgColor="E2F7E5"),  # 10
    ]
    FONT_DARK  = Font(bold=True, color="FFFFFF", size=10)
    FONT_LIGHT = Font(bold=False, color="000000", size=10)
    ...
    for r_idx, row in enumerate(rows, 1):
        ...
        if isinstance(rank, int):
            ...
            if rank_row <= 10:
                ...
                font = FONT_DARK if rank_row <= 6 else FONT_LIGHT
                for c in range(1, n_cols + 1):
                    ws.cell(row=r_idx, column=c).fill = fill
...
def main() -> None:
    sep = "=" * 65
    print(f"\n{sep}")
    print("  generate_previsoes_v6.py — Copa 2026 Excel v6")
    print("  Modelo: XGBoost v9 (ELO-based gls_ponderado + forte threshold)")
    print(sep)
    ...
    if missing:
        ...
        for t in missing:
            team_feats[t] = {"shots_on_goal_decay": 3.0, "gols_sofridos_decay": float("nan"),
                             "saves_decay": 1.8, "host_factor": 0.0}
    ...
    lam_cache   = lam_full  # alias para build_grupos_rows / build_classificacao_rows
    ...
    mc_csv        = Path("outputs/copa2026_previsoes_v6.csv")
    prob_rows     = build_probabilidades_rows(mc_csv)
    ...
    if prob_rows:
        ws4 = wb.create_sheet("Probabilidades MC")
        write_probabilidades(ws4, prob_rows)
