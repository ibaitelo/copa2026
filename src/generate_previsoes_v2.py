"""
generate_previsoes_v2.py — Gera copa2026_previsoes_v2.xlsx com dados do modelo v5.
...
OUT_XLSX = Path("outputs/copa2026_previsoes_v2.xlsx")
...
def build_grupos_rows(lam_cache: dict, elo: dict) -> list[list]:
    rows: list[list] = []
    rows.append(["COPA DO MUNDO 2026 — PREVISÃO DE PLACARES  (modelo XGBoost v5)"] + [None] * 14)
...
def build_matamata_rows(lam_cache: dict, elo: dict,
    ...
    rows: list[list] = []
    rows.append(["COPA DO MUNDO 2026 — MATA-MATA DETERMINÍSTICO  (modelo XGBoost v5)"] + [None] * 14)
    ...
    _add_phase("OITAVAS",    [r[0] for r in R32])
    _add_phase("QUARTAS",    [89, 90, 91, 92, 93, 94, 95, 96])
    _add_phase("SEMI",       [97, 98, 99, 100])
    _add_phase("SEMI-FINAL", [101, 102])
...
def write_matamata(ws, rows: list[list]) -> None:
    ...
    PHASE_FILLS = {
        "OITAVAS":    PatternFill("solid", fgColor="E2EFDA"),
        "QUARTAS":    PatternFill("solid", fgColor="FFF2CC"),
        "SEMI":       PatternFill("solid", fgColor="FCE4D6"),
        "SEMI-FINAL": PatternFill("solid", fgColor="F4B8C1"),
        "3º Lugar": PatternFill("solid", fgColor="E2EFDA"),
        "FINAL":      PatternFill("solid", fgColor="FFD700"),
    }
    PHASE_FONTS = {
        "OITAVAS":    Font(bold=True, color="375623", size=10),
        "QUARTAS":    Font(bold=True, color="7F6000", size=10),
        "SEMI":       Font(bold=True, color="843C0C", size=10),
        "SEMI-FINAL": Font(bold=True, color="99002B", size=10),
...
def main() -> None:
    ...
    print("  Modelo: XGBoost v5 (decay temporal ξ=0.001)")
    print("=" * 65)
