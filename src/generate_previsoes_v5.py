"""
generate_previsoes_v5.py — Gera copa2026_previsoes_v5.xlsx com dados do modelo v8.

Estrutura:
  - "Fase de Grupos"        : 48 jogos com λ, V%/E%/D%, top-3 placares (v8)
  - "Mata-Mata"             : bracket determinístico v8
...
OUT_XLSX = Path("outputs/copa2026_previsoes_v5.xlsx")
...
def build_grupos_rows(lam_cache: dict, elo: dict) -> list[list]:
    rows: list[list] = []
    rows.append(["COPA DO MUNDO 2026 — PREVISÃO DE PLACARES  (modelo XGBoost v8 — feature redesign + host_factor)"] + [None] * 14)
...
def build_matamata_rows(lam_cache: dict, elo: dict,
    ...
    rows: list[list] = []
    rows.append(["COPA DO MUNDO 2026 — MATA-MATA DETERMINÍSTICO  (modelo XGBoost v8 — feature redesign + host_factor)"] + [None] * 14)
...
def build_probabilidades_rows(mc_csv: Path) -> list[list]:
    ...
    rows: list[list] = []
    rows.append(["COPA DO MUNDO 2026 — PROBABILIDADES MONTE CARLO  (10.000 simulações — modelo v8)"] + [None] * 8)
...
def write_probabilidades(ws, rows: list[list]) -> None:
    ...
    # Gradient de cores por % campeão (top-10 = verde mais escuro)
    FILLS_RANK = [
...
def main() -> None:
    sep = "=" * 65
    print(f"\n{sep}")
    print("  generate_previsoes_v5.py — Copa 2026 Excel v5")
    print("  Modelo: XGBoost v8 (feature redesign + host_factor)")
    print(sep)
    ...
    mc_csv        = Path("outputs/copa2026_previsoes_v5.csv")
